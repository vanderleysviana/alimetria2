...
export function renderMeals(){
  ...
  if(!canEdit){
    ...
    empty.textContent = 'Selecione um paciente e uma dieta para começar.';
...
document.addEventListener('DOMContentLoaded', ()=>{
  ...
  setInterval(()=>{
    ...
    if(state.unsavedChanges){
      if(!obs.textContent.includes(' • Alterações não salvas')) obs.textContent = (state.patient?.name? `${state.patient.name} — ${state.editingDiet ? state.editingDiet.dietName||'' : ''}`.trim() : '— nenhum paciente selecionado —') + ' • Alterações não salvas';
    }else{
      if(state.patient && state.patient.name){
        obs.textContent = state.editingDiet ? `${state.patient.name} — ${state.editingDiet.dietName||''}` : `${state.patient.name} — ${state.patient.objetivo||''}`;
      }else{
